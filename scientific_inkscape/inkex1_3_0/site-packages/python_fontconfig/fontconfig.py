...
fc = ct.cdll.LoadLibrary(os.path.expanduser("~/.local/lib/libfontconfig.so.1"))

libc = ct.cdll.LoadLibrary("libc.so.6")


class FC:
    "useful definitions from fontconfig/*.h. You will need to use the constants," " but apart from that, see the more Pythonic wrappers defined outside this" " class in preference to accessing low-level structures directly."
...
class LangSet:
    "wrapper for FcLangSet objects. Do not instantiate directly: use the create, copy," " union and difference methods."
    ...
    def has(self, lang):
        "returns one of FcLangEqual, FcLangDifferentCountry" "/FcLangDifferentTerritory or FcLangDifferentLang."
...
class ObjectSet:
    "wrapper around FcObjectSet objects. For internal use only: all relevant" " functions will pass and return Python sequences."
...
class Range:
    "wrapper for FcRange objects, which store a pair of double values." " Do not instantiate directly; use the create method. Actually, probably" " best not to use this type at all. Properties of TypeRange will be returned" " as (low, high) tuples, and addition of such properties to a Pattern expects" " them to be supplied in the same form."
...
class Blanks:
    "wrapper for FcBlanks objects, which represent a set of character codes which" " are expected to be blank. These are used, for example, when scanning fonts," " to distinguish blank characters from unmapped ones.\n" "\n" "Do not instantiate directly: use the create method.\n" "\n" "Note the methods available are very limited, e.g. no enumeration of members or" " determination of cardinality."
...
class Config:
    "high-level wrapper around FcConfig objects. Do not instantiate directly;" " use the create or get_current methods."
    ...
    def init_load(celf, and_fonts):
        "creates a new instance of the default config, optionally including" " information about the actual available fonts."
        return celf(
            (
                fc.FcInitLoadConfig,
    ...
    def get_fonts(self, set_name):
        "returns one of the two sets of fonts, where set_name is either FC.SetSystem" " or FC.SetApplication."
    ...
    def font_list(self, pat, props):
        "finds all fonts matching Pattern pat, and returns a tuple of" " Patterns describing them, containing only the properties named" " in props."
        if not isinstance(pat, Pattern):
            raise TypeError("pat must be a Pattern")
        # end if
        if props != None:
...
class CharSet:
    "wrapper around FcCharSet objects. For internal use only: all relevant" " functions will pass and return Python sequences."
...
class FontSet:
    "wrapper around FcFontSet objects. For internal use only: all relevant" " functions will pass and return Python sequences."
...
class Matrix:
    ...
    def __repr__(self):
        return "%s(xx = %.3g, xy = %.3g, yx = %.3g, yy = %.3g)" % (
            type(self).__name__,
            self.xx,
            self.xy,
            self.yx,
...
class Pattern:
    "wrapper around FcPattern objects. Do not instantiate directly; use create" " or name_parse methods."
    ...
    def freetype_query(celf, filename, id, blanks):
        "constructs a Pattern representing the font with index id in the font file" " with the given name. Also returns the count of fonts in the file."
    ...
    if freetype != None:
        ...
        def freetype_query_face(celf, face, filename, id, blanks):
            "constructs a Pattern representing the given freetype2.Face. It is assumed" " to have been the font with index id loaded from the specified file name;" " this information is included in the pattern."
    ...
    def __repr__(self):
        props = tuple(q for q in (self.get(PROP.FAMILY, 0)[0],) if q != None) + tuple(
            ":%s=%s" % (p.value, q)
            for p in (
                PROP.STYLE,
                PROP.SLANT,
...
class StrSet:
    "wrapper around FcStrSet objects. For internal use only: all relevant" " functions will pass and return Python sets."
...
class StrList:
    "wrapper around FcStrList objects. For internal use only: all relevant" " functions will pass and return sequences of Python strings."
